...
ALLOWED_STATE = ("Shift", "Lock", "Control", "Mod1", "Mod2", "Mod3", "Mod4", "Mod5")
MS_DELAY = 1
...
class _TerminalFunctionality:
    ...
    def _limit_backspace(self):
        ...
        def get_initial(char):
            return self.get(
                str(insert_idx.row) + ".0", str(insert_idx.row) + f".{char}"
            )
    ...
    def _get_commands(self, _cmd=None, _input=None):
        ...
        def initial_index(ln):
            ...
            index_col = 0
            base_index = (
                str(self.index(f"end-{ln}l").row) + str(".") + str(len(self._basename))
            )
    ...
    def _set_initials(self, evt=None):
        """Internal function"""
        if self.get("end-2c") == "\\" or self.get("end-1l", "end-1c") == "> ":
    ...
    def _on_keypress(self, evt=None):
        ...
        if (
            self._ignore_keypress(evt)  # Avoid typing on basename and spacer
            or (
                (insert_idx.column <= self._limit_backspace or False)
                and evt.keysym == "Left"
            )  # Avoid to get on basename and spacer
            # or (evt.keysym in AVOID_KEYSYM) # break if keysym not allowed
            or (
                evt.keysym not in ALLOWED_KEYSYM
    ...
    def _on_backspace(self, evt=None):
        ...
        insert_idx = self.index("insert")
        if (
            insert_idx.column <= self._limit_backspace
...
class Terminal(Text, _TerminalFunctionality):
    ...
    def get_output(self):
        ...
        if self._out or self._err:
            return {"error": self._err, "output": self._out}
